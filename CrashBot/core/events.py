...
class EventBus:
    ...
    def __init__(self, keep_history: int = 100):
        ...
        Args:
            keep_history: Quantidade de eventos recentes a manter (0 = desabilitado)
        """
        self._subscribers: Dict[BotEvent, List[EventCallback]] = {}
        self._global_subscribers: List[EventCallback] = []  # Observam TODOS eventos
    ...
    def subscribe(
        self, event: BotEvent, callback: EventCallback, priority: int = 0
    ) -> Callable[[], None]:
        """
        ...
        Args:
            ...
            priority: Prioridade (maior = executa primeiro) [não implementado ainda]

        Returns:
            Função para cancelar a inscrição (unsubscribe)

        Example:
            unsubscribe = bus.subscribe(BotEvent.BALANCE_CHANGED, handler)
            # Mais tarde...
            unsubscribe()  # Remove o handler
        """
        with self._lock:
            ...
            if callback not in self._subscribers[event]:
                ...
                logger.debug(
                    f"Subscriber registrado: {event.name} -> {callback.__name__}"
                )

        # Retorna função para unsubscribe
        def unsubscribe():
            self._unsubscribe(event, callback)

        return unsubscribe

    def subscribe_all(self, callback: EventCallback) -> Callable[[], None]:
        """
        Registra um callback para TODOS os eventos.
        ...
        with self._lock:
            if callback not in self._global_subscribers:
                ...
                logger.debug(f"Global subscriber registrado: {callback.__name__}")
    ...
    def _unsubscribe(self, event: BotEvent, callback: EventCallback) -> None:
        """Remove um callback de um evento."""
        with self._lock:
            if event in self._subscribers and callback in self._subscribers[event]:
                self._subscribers[event].remove(callback)
                logger.debug(f"Subscriber removido: {event.name}")

    def on(self, event: BotEvent) -> Callable[[EventCallback], EventCallback]:
        """
    ...
    def publish(
        ...
        Args:
            ...
            payload: Dados do evento (dicionário)
        ...
        for callback in all_subscribers:
            try:
                ...
                logger.error(
                    f"Erro no handler {callback.__name__} para {event.name}: {e}",
                    exc_info=True,
                )

        logger.debug(
            f"Evento publicado: {event.name} ({len(all_subscribers)} handlers)"
        )
        return event_data
    ...
    def get_history(
        self, event_type: Optional[BotEvent] = None, limit: int = 50
    ) -> List[EventData]:
        """
...
def emit(event: BotEvent, **kwargs) -> EventData:
    """
...
def emit_async(event: BotEvent, **kwargs) -> None:
    """
...
if __name__ == "__main__":
    ...
    @bus.on(BotEvent.BALANCE_CHANGED)
    def handle_balance(data: EventData):
        print(f"[HANDLER] Saldo mudou: R$ {data['old']:.2f} → R$ {data['new']:.2f}")
    ...
    print(
        f"Subscribers BALANCE_CHANGED: {bus.get_subscriber_count(BotEvent.BALANCE_CHANGED)}"
    )
