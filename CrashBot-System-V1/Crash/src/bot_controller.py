...
class BotController:
    ...
    def __init__(self, config_filename="config.json"):
        ...
        self._meta_percentual: float = 0.30  # Meta definida pelo usuário
    ...
    def _perguntar_configuracoes_sessao(self) -> RiskMode:
        """Coleta o modo de risco e a meta de lucro do usuário."""
        self.console.print("\n[bold cyan]━━━ CONFIGURAÇÃO DA SESSÃO ━━━[/bold cyan]")
        self.console.print("\n[bold yellow]🎯 ESCOLHA SEU MODO DE RISCO:[/bold yellow]")
        self.console.print("")
        self.console.print(
            "  [blue]1. ULTRA CONSERVADOR[/blue] - 9 velas, risco mínimo"
        )
        self.console.print("")
        self.console.print(
            "  [green]2. CONSERVADOR[/green] - 8 velas, menor risco"
        )
        self.console.print("")
        self.console.print(
            "  [yellow]3. MODERADO[/yellow] - 7 velas, risco equilibrado"
        )
        self.console.print("")
        self.console.print(
            "  [red]4. AGRESSIVO[/red] - 6 velas, maior risco"
        )
        self.console.print("")
        ...
        risk_mode = self._obter_escolha_valida(
            prompt="Escolha (1-4): ",
            opcoes={
                "1": RiskMode.ULTRA_CONSERVADOR,
                "2": RiskMode.CONSERVADOR,
                "3": RiskMode.MODERADO,
                "4": RiskMode.AGRESSIVO,
            },
        )

        mode_colors = {
            RiskMode.ULTRA_CONSERVADOR: "blue",
        ...
        # Pede a meta de lucro ao usuário
        self.console.print(
            "\n[bold yellow]💰 DEFINA SUA META DE LUCRO (%):[/bold yellow]"
        )
        self.console.print(
            "  Ex: 30 para 30%, 50 para 50%, 100 para 100%"
        )
        self._meta_percentual = self._obter_meta_usuario()
        self.console.print(
            f"\n✅ Meta definida: [bold green]"
            f"{self._meta_percentual:.0%}[/bold green]",
            style="bold",
        )

        return risk_mode

    def _obter_meta_usuario(self) -> float:
        """Pede ao usuário a meta de lucro em porcentagem."""
        while True:
            try:
                valor = self.console.input(
                    "[green]Meta de lucro (%): [/green]"
                )
                meta = float(valor.strip().replace(",", "."))
                if meta <= 0:
                    self.console.print(
                        "A meta deve ser maior que 0!",
                        style="red",
                    )
                    continue
                if meta > 1000:
                    self.console.print(
                        "Meta muito alta! Máximo 1000%.",
                        style="red",
                    )
                    continue
                return meta / 100.0  # Converte para decimal
            except ValueError:
                self.console.print(
        ...
        while self.running:
            try:
                if msvcrt.kbhit():
                    ...
                    if key == "1":
                        self._trocar_modo_runtime(RiskMode.ULTRA_CONSERVADOR)
                    elif key == "2":
    ...
    def _run_main_loop(self):
        ...
        self.strategy.iniciar_sessao(
            ...
            meta_percentual=self._meta_percentual,
        )
        self.running = True
        self._send_telemetry(
            tipo="sessao_inicio",
