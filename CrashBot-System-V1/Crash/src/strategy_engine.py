...
RISK_MODE_CONFIG = {
    RiskMode.ULTRA_CONSERVADOR: {
        "banca_percent": 1.00,  # Usa 100% da banca real
        "gatilho_opcoes": [9],  # Sempre 9 velas
    },
    ...
    RiskMode.MODERADO: {
        "banca_percent": 1.00,  # Usa 100% da banca real
        "gatilho_opcoes": [7],  # Sempre 7 velas
    },
    RiskMode.AGRESSIVO: {
        "banca_percent": 1.00,  # Usa 100% da banca real
        "gatilho_opcoes": [6],  # Sempre 6 velas
    },
}
...
class MLHighConfidencePolicy(StrategyPolicy):
    ...
    def check_trigger(self, history: deque) -> Tuple[bool, str]:
        """
        Calcula a confiança do ML apenas para exibição.
        NUNCA ativa apostas nem pula jogadas — é apenas visual.
        Retorna: (False, mensagem_info: str)
        """
        if not self.le:
            return False, self.ultima_decisao
        ...
        if probability is None:
            self.last_calculated_prob = 0.0
            self._set_decision(
                "📊 ML: Erro na predição", "aguardando", False
            )
            return False, self.ultima_decisao

        self.last_calculated_prob = probability

        # Apenas exibe a confiança — nunca ativa nem pula
        if probability >= self.confidence_threshold:
            self._set_decision(
                f"📊 ML: Confiança ALTA {probability:.1%} "
                f"(ref. {self.confidence_threshold:.0%})",
                "aguardando",
                False,
            )
        else:
            self._set_decision(
                f"📊 ML: Confiança {probability:.1%} "
                f"(ref. {self.confidence_threshold:.0%})",
                "aguardando",
                False,
            )
    ...
    def get_bet_recommendation(
        ...
        # ML é apenas visual — nunca gera recomendação de aposta
        return None

    def process_result(self, explosion_value: float):
        # ML é apenas visual — não processa resultados
        pass
...
class StrategyEngine:
    ...
    def iniciar_sessao(
        self, banca_inicial: float, risk_mode: RiskMode, meta_percentual: float = 0.30
    ):
        """
        Inicia a sessão com o modo de risco escolhido.
        A meta de lucro é definida pelo usuário.
        """
        ...
        # Meta de lucro definida pelo usuário
        self.meta_lucro_percentual = meta_percentual
        ...
        logger.info(
            f"Meta de Lucro (usuário): {self.meta_lucro_percentual:.1%} "
    ...
    def _reiniciar_ciclo_pos_meta(self, saldo_atual: float):
        ...
        Mantém a meta de lucro definida pelo usuário.
        """
        ...
        # Mantém a mesma meta do usuário (não sorteia nova)

        logger.info(
            f"CICLO REINICIADO - Modo {self.risk_mode.name} | "
            f"Nova Banca Op.: R$ {self.banca_inicial:.2f} | "
            f"Meta mantida: {self.meta_lucro_percentual:.1%}"
        )
    ...
    def trocar_modo(self, novo_modo: RiskMode, saldo_atual: float):
        ...
        Reseta a sessão mantendo o saldo atual e a meta do usuário.
        """
        ...
        # Mantém a meta do usuário na troca de modo
        meta_atual = self.meta_lucro_percentual or 0.30

        # Reinicia sessão com novo modo e mesma meta
        self.iniciar_sessao(saldo_atual, novo_modo, meta_atual)
