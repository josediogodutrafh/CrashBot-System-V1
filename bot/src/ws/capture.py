...
class CrashWSCapture:
    ...
    def __init__(self, port: int = 9222, auto_reconnect: bool = True,
        ...
        self._parser = parser  # BaseParser instance (keywords + parse_frame)
        ...
        # Chrome process PID (for safe per-instance kill)
    ...
    def _kill_chrome(self) -> bool:
        """Fecha o Chrome desta instância (por PID, não mata outros).

        Se não tiver PID registrado, faz fallback para matar todos
        (comportamento original, compatibilidade single-instance).
        """
        try:
            if self._chrome_pid:
                logger.info(
                    "Fechando Chrome PID %d (porta %d)...",
                    self._chrome_pid, self.port,
                )
                subprocess.run(
                ...
                time.sleep(2)
                return not self._is_chrome_debug_running()
            else:
                # Fallback: matar todos (single-instance compat)
                logger.info("Fechando TODOS os processos Chrome (sem PID)...")
                subprocess.run(
    ...
    def _get_bot_profile_dir(self) -> Path:
        ...
        Multi-instance: each port gets its own profile directory to allow
        multiple Chrome instances (one per platform) simultaneously.
        """
        if getattr(sys, "frozen", False):
            base = Path(sys.executable).parent
        else:
            base = Path(__file__).parent.parent.parent
        # Port-specific dir for multi-instance (9222 keeps original name for compat)
        if self.port == 9222:
            profile = base / "crashbot_chrome_data"
        else:
            profile = base / f"crashbot_chrome_data_{self.port}"
    ...
    def ensure_chrome_running(
        ...
        # ANTI-DETECCAO: flags para minimizar fingerprint de automacao
        # IMPORTANTE: --remote-debugging-port ainda eh detectavel, isso eh
        # mitigacao parcial, nao imunidade total.
        realistic_ua = (
        ...
        try:
            ...
            logger.info("Chrome PID: %d (porta %d)", proc.pid, self.port)
        except Exception as e:
    ...
    def _find_game_tab(self) -> Optional[str]:
        ...
        try:
            ...
            # Keywords: parser-specific ou fallback global
            keywords = (
                self._parser.game_tab_keywords
                if self._parser else GAME_TAB_KEYWORDS
            )
    ...
    def _handle_parsed_event(self, evt):
        """Converte ParsedEvent do parser em atualizações de estado + emissão.

        Mapeia os tipos de evento padronizados para os handlers internos
        do capture, mantendo a mesma interface de callbacks para o bot.
        """
        t = evt.event_type
        d = evt.data

        if t == "round_end":
            crash_value = d.get("crash", 0.0)
            ...
            result = RoundResult(
                ...
                begin_time=d.get("begin_time", 0),
                end_time=d.get("end_time", 0),
            )
            self.round_results.append(result)
            self._set_phase(GamePhase.CRASHED)
            self._emit("round_end", d)

            logger.info(
                f"Round {round_id}: crash={crash_value:.2f}x "
                f"(duration={duration}s) [parser]"
            )

        elif t == "betting_phase":
            self.bet_left_seconds = d.get("bet_left_seconds", 0)
            self._set_phase(GamePhase.BETTING)
            self._emit("betting_phase", d)

        elif t == "game_start":
            self.current_multiplier = 1.0
            self._set_phase(GamePhase.PLAYING)
            self._emit("game_start", d)

        elif t == "pre_start":
            self._set_phase(GamePhase.PRE_START)
            self._emit("pre_start", d)

        elif t == "balance_update":
            ...
            if balance > 0:
                ...
                self._emit("balance_update", {
                    ...
                    "diff": balance - old_balance if old_balance > 0 else 0,
                })

        elif t == "bet_placed":
            self._emit("bet_placed", d)
            ...
            if multiplier > 0:
                ...
                self._emit("multiplier_update", d)
    ...
    def _process_game_frame(self, payload: str, opcode: int = 1):
        ...
        Se um parser foi injetado, delega o parsing para ele e emite
        os eventos padronizados. Caso contrário, usa o parsing inline
        legado (protocolo Brabet hardcoded - backward compat).
        ...
        # ── Parser-delegated mode ──────────────────────────────────
        if self._parser:
            try:
                events = self._parser.parse_frame(payload, opcode)
                for evt in events:
                    self._handle_parsed_event(evt)
            except Exception as e:
                logger.error(
                    f"Parser '{self._parser.platform_name}' erro: {e}"
                )
                self._stats.errors += 1
            return

        # ── Legacy inline parsing (Brabet compat) ──────────────────
        msg = None
