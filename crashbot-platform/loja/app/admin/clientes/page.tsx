  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400">{clientes.length} cliente{clientes.length !== 1 ? "s" : ""} cadastrado{clientes.length !== 1 ? "s" : ""}</p>
      </div>
        ...
        {filteredClientes.map((cliente) => (
          ...
          >
            ...
            <div className="mt-4 pt-4 border-t border-purple-900/30 flex gap-4">
              <div>
                <p className="text-2xl font-bold text-white">{cliente.totalLicencas}</p>
                <p className="text-xs text-gray-500">Licencas</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-400">{cliente.licencasAtivas}</p>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#12121a] rounded-2xl border border-purple-900/30 w-full max-w-2xl max-h-[80vh] overflow-hidden">
            <div className="p-6 border-b border-purple-900/30 flex items-center justify-between">
              ...
              <button onClick={() => setSelectedCliente(null)} className="text-gray-400 hover:text-white transition-colors">
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              <h3 className="text-lg font-semibold text-white mb-4">Licencas ({selectedCliente.licencas.length})</h3>
              <div className="space-y-3">
                {selectedCliente.licencas.map((licenca) => (
                  <div key={licenca.id} className="bg-[#0a0a0f] rounded-xl p-4 border border-purple-900/20">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      <span className="px-2 py-0.5 bg-purple-500/20 text-purple-400 rounded">{licenca.plano_tipo}</span>
                      <span>Criada: {formatDate(licenca.created_at)}</span>
                      {!licenca.esta_expirada && (
                        <span className={licenca.dias_restantes <= 7 ? "text-red-400" : ""}>{licenca.dias_restantes} dias restantes</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="p-6 border-t border-purple-900/30 flex gap-3">
              {selectedCliente.whatsapp && selectedCliente.whatsapp !== "Nao informado" && (
                
                  href={`https://wa.me/55${selectedCliente.whatsapp.replace(/\D/g, "")}`}
                  target="_blank"
              ...
              <button onClick={() => setSelectedCliente(null)} className="flex-1 px-4 py-2 text-gray-400 hover:text-white border border-purple-900/30 rounded-xl transition-colors">
                Fechar
              </button>
            </div>
          </div>
